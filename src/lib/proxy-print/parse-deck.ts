...
const GAME_SECTION = /^#(yugioh|pokemon|digimon|onepiece)\b/i;
const GAME_BRACKET = /^\[(yugioh|pokemon|digimon|onepiece)\]\s*/i;
const GAME_PREFIX = /^(yugioh|pokemon|digimon|onepiece):\s*/i;
const ART_HINT_TOKENS = new Set(["fa", "aa", "sp", "foil", "textured", "tex", "alt"]);
...
function extractCustomImage(body: string): { body: string; customImageUrl: string | null } {
  const pipe = body.match(/\s*\|\s*(https?:\/\/\S+)\s*$/i);
  if (pipe) {
    return { body: body.slice(0, pipe.index).trim(), customImageUrl: pipe[1] };
  }
  const atUrl = body.match(/\s+@\s*(https?:\/\/\S+)\s*$/i);
  if (atUrl) {
...
function parseLineGame(
  ...
  if (section) {
    const g = section[1].toLowerCase() as ProxyGame;
    if (PROXY_GAMES.includes(g)) return { line: "", game: g, isGameHeader: true };
  }

  const bracket = line.match(GAME_BRACKET);
  if (bracket) {
    const g = bracket[1].toLowerCase() as ProxyGame;
    if (PROXY_GAMES.includes(g)) {
  ...
  if (prefix) {
    const g = prefix[1].toLowerCase() as ProxyGame;
    if (PROXY_GAMES.includes(g)) {
...
export function hasMixedGameSections(text: string): boolean {
  ...
  for (const raw of text.replace(/\r\n/g, "\n").split("\n")) {
    ...
    if (m && PROXY_GAMES.includes(m[1].toLowerCase() as ProxyGame)) {
      games.add(m[1].toLowerCase() as ProxyGame);
    }
  }
  return games.size > 1;
}
...
export function parseTextDeckContent(text: string, defaultGame: ProxyGame): Record<string, DeckEntry[]> {
  ...
  for (const raw of text.replace(/\r\n/g, "\n").split("\n")) {
    ...
    if (line.startsWith("#")) {
      ...
      if (gameHeader) {
        currentGame = gameHeader[1].toLowerCase() as ProxyGame;
        zones[section] ??= [];
        continue;
      }
...
export function deckLineWithVariantImage(
  ...
  const normalizeLine = (line: string) =>
    line.trim().replace(/\s*\|\s*https?:\/\/\S+\s*$/i, "").trim();
